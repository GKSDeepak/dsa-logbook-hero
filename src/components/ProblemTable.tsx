...
export function ProblemTable({ problems, sessionType, onUpdateProblem, onAddProblem, onDeleteProblem }: ProblemTableProps) {
  const [editingTagProblemId, setEditingTagProblemId] = useState<string | null>(null);
  ...
  const handleAddTag = (problem: Problem) => {
    if (newTag.trim() === '') {
      setEditingTagProblemId(null);
      setNewTag('');
      return;
    }
      ...
      <div className="flex flex-wrap items-center gap-1">
        {tags.map((tag) => (
          ...
          >
            {tag}
            <button 
              onClick={() => handleRemoveTag(problem, tag)} 
        ...
        {editingTagProblemId === problem.id ? (
          <Input
            value={newTag}
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') {
                e.preventDefault();
                handleAddTag(problem);
              }
            }}
            onBlur={() => handleAddTag(problem)}
            autoFocus
          ...
          <Button
            variant="ghost"
            size="icon"
            onClick={() => setEditingTagProblemId(problem.id)}
